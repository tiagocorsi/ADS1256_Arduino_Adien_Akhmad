...
uint8_t ADS1256::getPGA() // Reading PGA from the ADCON register
{
  ...
  pgaValue = readRegister(ADS1256_RADD_ADCON) & 0b00000111;
...
long ADS1256::readSingle() // Reading a single value ONCE using the RDATA command
{
  ...
  digitalWrite(pinCS, LOW); // REF: P34: "CS must stay low during the entire command sequence"
  waitDRDY();
  ...
  digitalWrite(pinCS, HIGH); // We finished the command sequence, so we set CS to HIGH
  SPI.endTransaction();
